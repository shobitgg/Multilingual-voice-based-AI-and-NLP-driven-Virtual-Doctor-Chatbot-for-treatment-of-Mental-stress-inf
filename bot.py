import nltk # categorizing text, analyzing linguistic structure text processing  NLTK, is a suite of libraries and programs for symbolic and statistical natural language processing (NLP) for English written in the Python programming language.
import warnings #Warning messages are typically issued in situations where it is useful to alert the user of some condition in a program, where that condition (normally) doesn't warrant raising an exception and terminating the program.
...
sent_tokens = nltk.sent_tokenize(raw)# converts to list of sentences ,uses an instance of PunktSentenceTokenizer from the nltk.
word_tokens = nltk.word_tokenize(raw)# converts to list of words , It actually returns the syllables from a single word.
...
Introduce_Ans = ["My name is Covid-Dev-Bot.","My name is Covid-Dev-Bot you can call me Codev.","Im Covid-Dev-Bot :) ","My name is Covid-dev-Bot. and my nickname is codev and i am happy to solve your queries :) "]
...
Basic_Om = ("what is time","what is module.","what is module ","what is module ?","what is module?","what is module in python","what is module in python.","what is module in python?","what is module in python ?")
...
from sklearn.feature_extraction.text import TfidfVectorizer #Transforms text to feature vectors that can be used as input to estimator to get a feature index.
#
from sklearn.metrics.pairwise import cosine_similarity #Cosine similarity measures the similarity between two vectors of an inner product space.
...
def response(user_response):
    ...
    flat = vals.flatten()  #The flatten() function is used to get a copy of an given array collapsed into one dimension.
    flat.sort()
    req_tfidf = flat[-2]
    if(req_tfidf==0):
