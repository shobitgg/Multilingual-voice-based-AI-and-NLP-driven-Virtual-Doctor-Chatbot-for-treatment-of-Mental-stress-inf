...
from bot import chat #bot.py
import pyttsx3 #TEXT TO SPEECH  #mp3  #TTS
import threading
...
# Menu bar
    ...
    # Options
       ...
       # color_theme.add_command(label='Mkbhd',command=self.MKBHD)
        ...
        help_option.add_command(label="About Shobit-Bot", command=self.msg)
        ...
        # send button
        self.send_button = Button(self.send_button_frame, text="Send", width=5, relief=GROOVE, bg='red',
    ...
    def playResponce(self,responce):
        ...
        if len(responce) > 100:
            if responce.find('--') == -1:
                b = responce.split('--')
                print(b)
        #x.setProperty("language",'hi')
        x.setProperty('rate',120)
        x.setProperty('volume',300)
        x.say(responce)
        x.runAndWait()
...
#os.system("start voice.mp3")
    ...
    def msg(self):
        tkinter.messagebox.showinfo("Shobit-bot v1.0",'Shobit-Bot is a chatbot for answering python queries\nIt also answers some corona related qanda .It is based on retrival-based NLP using pythons NLTK tool-kit module\nGUI is based on Tkinter\nIt can answer questions regarding python language for new learners')

    def about(self):
        tkinter.messagebox.showinfo("Shobit-Bot Developer","1.SHOBIT G VIT  VELLORE ")
    ...
    def send_message_insert(self, message):
        ...
        self.text_box.configure(state=DISABLED)
        self.text_box.see(END)
        #r=transliterate(ob, sanscript.IAST, sanscript.KANNADA)
        translator= Translator(from_lang="english",to_lang="kannada")
        r = translator.translate(ob)
        pr="SHOBIT(K): " + r + "\n"
        self.text_box.configure(state=NORMAL)
        self.text_box.insert(END, pr)
        self.text_box.configure(state=DISABLED)
        ...
        rs=transliterate(ob, sanscript.HK, sanscript.TELUGU)
    ...
    def color_theme_dark_blue(self):
        ...
        self.send_button_frame.config(bg="red")
...
root.title("SHOBIT-Bot                <|))) VOICE OUTPUT ENABLED >))")
